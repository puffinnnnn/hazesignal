...
The four-month validation pull contained 98,577 hotspot rows, 2,928 hourly wind readings, and 117 daily PM2.5 readings. The large raw files remain local; `validation_2023.csv` contains the 122 daily rows required by `npm run validate`. Recreate the source pulls by running the three fetch commands with `2023-09-01 2023-12-31`, then run the analysis with those matching files.
